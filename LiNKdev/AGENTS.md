...
Product rules: `.cursor/rules/`. Local program stub: `product/programs/linksites/` when present (execution metadata only).

## Execution target (do not wire here)

This repo is the **LinkSites suite product** execution target — not the LiNKdev program host.
