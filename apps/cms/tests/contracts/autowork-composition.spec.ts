...
describe('actual CMS to LiNKautowork composition', () => {
  ...
  const liveEnv = (directory: string) => {
    const signingSecret = process.env.LINKAUTOWORK_SIGNING_SECRET
    const outboxIntegritySecret = process.env.LINKAUTOWORK_OUTBOX_INTEGRITY_SECRET
    process.env.LINKAUTOWORK_GATEWAY_URL = 'http://127.0.0.1:1/events'
    process.env.LINKAUTOWORK_SIGNING_SECRET = signingSecret ?? 'ltfx.auto.linkautowork_signing_secret.bed6dcd92148.v1'
    ...
    process.env.LINKAUTOWORK_OUTBOX_INTEGRITY_SECRET = outboxIntegritySecret ?? 'ltfx.auto.linkautowork_outbox_integrity_secret.c894e74e9421.v1'
    ...
    process.env.LINKAUTOWORK_RECEIPT_CONTRACT = '2026-08-13.v1'
    process.env.LINKAUTOWORK_CLAIM_CONTRACT = 'platform.auth-claims/1.1.0'
    process.env.LINKAUTOWORK_ORG_ID = 'org_demo'
  }
  ...
  it('fails closed and retries when the gateway omits its acknowledgement receipt', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'linksites-cms-w2-05-'))
    liveEnv(directory)
    process.env.LINKAUTOWORK_SIGNING_SECRET = 'ltfx.auto.linkautowork_signing_secret.6e8489151d45.v1'
    process.env.LINKAUTOWORK_OUTBOX_INTEGRITY_SECRET = 'ltfx.auto.linkautowork_outbox_integrity_secret.2981ff9f6d7c.v1'
    const req = { payload: { findByID: vi.fn().mockResolvedValue({ id: 'site-1', orgId: 'org_demo', programId: 'program-1', leadId: 'lead-1' }) } } as never
    await triggerLiNKautowork({ id: 'page-1', collection: 'pages', eventType: 'content_published', site: 'site-1', req }, { readProgramPass: vi.fn().mockResolvedValue({ state: 'PASS', completionId: 'pass-1', programId: 'program-1', orgId: 'org_demo', leadId: 'lead-1', siteId: 'site-1' }) })
  ...
  it('fails closed for incomplete live handoff without calling transport', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'linksites-cms-w2-05-'))
    liveEnv(directory)
    delete process.env.LINKAUTOWORK_ISSUER
    const fetchMock = vi.spyOn(globalThis, 'fetch')
    const req = { payload: { findByID: vi.fn().mockResolvedValue({ id: 'site-1', orgId: 'org_demo', programId: 'program-1', leadId: 'lead-1' }) } } as never
    await expect(triggerLiNKautowork({ id: 'page-1', collection: 'pages', eventType: 'content_published', site: 'site-1', req }, { readProgramPass: vi.fn().mockResolvedValue({ state: 'PASS', completionId: 'pass-1', programId: 'program-1', orgId: 'org_demo', leadId: 'lead-1', siteId: 'site-1' }) })).rejects.toThrow(/issuer|incomplete|LiveMode/i)
    expect(fetchMock).not.toHaveBeenCalled()
  })
})
