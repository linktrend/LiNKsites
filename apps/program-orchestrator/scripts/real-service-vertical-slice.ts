...
try {
  ...
  const privateRecords = await fetch(`${cmsUrl}/api/pages?site=${encodeURIComponent(siteId)}&where[previewEnvironment][equals]=private-preview&where[status][equals]=published&draft=false&limit=100`, { headers: { Authorization: `users API-Key ${apiKey}` } }).then(async (response) => { assert.equal(response.status, 200); return response.json() as Promise<{ docs: Array<Record<string, unknown>> }> })
  const markedRecords = await fetch(`${cmsUrl}/api/pages?site=${encodeURIComponent(siteId)}&where[previewEnvironment][equals]=private-preview&where[promotionRunMarker][equals]=${encodeURIComponent(runMarker)}&where[status][equals]=published&draft=false&limit=100`, { headers: { Authorization: `users API-Key ${apiKey}` } }).then(async (response) => { assert.equal(response.status, 200); return response.json() as Promise<{ docs: Array<Record<string, unknown>> }> })
  ...
  const promotedDrafts = markedRecords.docs
  assert.equal(promotedDrafts.length, 5, 'the schema-backed W2-02 run marker must identify exactly its five promoted documents')
  assert.ok(promotedDrafts.every((doc) => doc.promotionRunMarker === runMarker), 'every promoted document must retain the exact schema-backed run marker')
  assert.ok(promotedDrafts.every((doc) => doc.status === 'published' && doc._status === 'published' && doc.previewEnvironment === 'private-preview' && doc.publicActivation === false), 'every marked W2-02 publication must be private-preview only')
  assert.ok(publishedPreviewRecords.docs.some((doc) => doc.promotionRunMarker !== runMarker && doc.status === 'published' && doc._status === 'published'), 'the W2-04 seeded published preview record must remain distinct from W2-02 drafts')
  ...
  try {
    ...
    await page.setExtraHTTPHeaders({ 'X-LiNKsites-Preview-Key': previewToken })
    const allowed = await page.goto(`${webUrl}/en/demo`, { waitUntil: 'domcontentloaded' })
    assert.equal(allowed?.status(), 200)
    assert.match(allowed?.headers()['x-robots-tag'] ?? '', /noindex/)
    assert.match(allowed?.headers()['cache-control'] ?? '', /no-store/)
    await page.locator('[data-private-preview="true"]').waitFor({ state: 'visible' })
    await page.getByText(runMarker, { exact: false }).waitFor({ state: 'visible' })
