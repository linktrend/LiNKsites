...
try {
  ...
  assert.ok(privateRecords.docs.some((doc) => !JSON.stringify(doc).includes(runMarker) && doc.status === 'published' && doc._status === 'published'), 'the W2-04 seeded published preview record must remain distinct from W2-02 drafts')
