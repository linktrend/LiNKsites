...
export function validateRuntimeConfig(config: RuntimeConfig): RuntimeConfig {
  ...
  if (!/^[a-f0-9]{40}$/i.test(config.libraryCommitSha) || !/^[a-f0-9]{64}$/i.test(config.libraryCatalogChecksum) || !/^[a-f0-9]{64}$/i.test(config.libraryEntryChecksum)) throw new Error('W2-02 requires one exact LiNKlibraries commit and catalog/entry checksums')
