...
export function validateRuntimeConfig(config: RuntimeConfig): RuntimeConfig {
  ...
  if (!nativeV2Provider && (!/^[a-f0-9]{40}$/i.test(config.libraryCommitSha) || !/^[a-f0-9]{64}$/i.test(config.libraryCatalogChecksum) || !/^[a-f0-9]{64}$/i.test(config.libraryEntryChecksum))) throw new Error('local-proof W2-02 requires one exact legacy LiNKlibraries commit and catalog/entry checksums')
...
export function createLocalConfig(baseDir: string, orgId = 'local-org'): RuntimeConfig {
  ...
  return { mode: 'local', orgId, statePath, intakePath: resolve(baseDir, 'leads.ndjson'), completionPath: resolve(baseDir, 'completions.ndjson'), approvedFactsPath: resolve(baseDir, 'approved-facts.json'), maxAttempts: 3, concurrency: 2, leaseDurationMs: 30_000, executingRevision: actualRevision(), executableCheckpoint: executableCheckpoint(), workerId: `w2-02:${process.pid}:${randomUUID()}`, payloadBaseUrl: '', payloadApiKey: '', payloadSiteId: '', webMasterBaseUrl: '', previewAccessToken: '', linkautoworkGatewayUrl: '', linkautoworkSigningSecret: '', linkautoworkSigningKeyId: '', linkautoworkEventGrants: '', commercialOutcomeGatewaySecret: '', commercialOutcomeGatewayKeyId: '', libraryRepositoryPath: process.env.W2_02_LIBRARY_REPOSITORY_PATH ?? '/Users/linktrend/Projects/LiNKlibraries', libraryCommitSha: 'a7193d40152747db2a03e094fa263f324a971a0b', libraryCatalogChecksum: '02d6d962d9b1e82fb898442d3de0833ded60be7f4eb177c84c176f0233ad6c0c', libraryEntryChecksum: '2ea7b6f004451c9f82b74892add71ae42164f5a03c25a8f0d5afdb310107417c', approvedExecutors: Object.fromEntries(W2_02_GRAPH.map((issue) => [issue.executorKind, issue.executorVersion])), approvedCapabilities: Object.fromEntries(Object.entries(EXECUTOR_BINDINGS).map(([kind, binding]) => [kind, [...binding.capabilities]])) }
}
