...
const PROJECTION_MARKER = 'linksites' as const
...
export type LinksitesOversightProjection = {
  ...
  metadata: {
    tenantId: string; domain: 'linksites'; domainToken: typeof PROJECTION_MARKER; contextKind: 'site' | 'program' | 'project'
...
export function buildLinksitesOversightProjection(input: LinksitesOversightInput): LinksitesOversightProjection {
  ...
  const projection: LinksitesOversightProjection = {
    ...
    metadata: { tenantId: ref(input.tenantId, 'tenantId'), domain: 'linksites', domainToken: PROJECTION_MARKER, contextKind: 'site', contextReference: ref(input.contextReference, 'contextReference'), ownerReference: ref(input.ownerReference, 'ownerReference'), confidence, retentionClass: 'governed', purpose: 'projection_exchange', region: text(input.region ?? 'global', 'region', 40).toLowerCase(), shareabilityState: 'tenant_only', sourcePrivacy: 'tenant_internal', destinationPrivacy: 'tenant_internal', authority: 'advisory', verticalAuthority: 'none' },
