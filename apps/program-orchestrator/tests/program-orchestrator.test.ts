...
test('production composition boots with complete approved local configuration', async () => {
  ...
  try {
    ...
    const configuredOutcomeSecret = value['config']['commercialOutcomeGatewaySecret']
    const gateway = new LiNKautoworkGateway({ secret: configuredOutcomeSecret, keyId: value.config.commercialOutcomeGatewayKeyId, environment: 'development', transport: async () => { throw new Error('not used') }, policies: [{ eventName: 'commercial.outcome.recorded', orgIds: [value.config.orgId], environments: ['development'] }] })
...
test('process termination after claim and irreversible receipt is reclaimed by a new worker', async () => {
  ...
  try {
    ...
    await restarted.ledger.createOrResume(lead('lead-process-crash'))
...
test('a paused stale worker cannot acknowledge an irreversible receipt after lease expiry and reclaim', async () => {
  ...
  try {
    ...
    await second.ledger.createOrResume(lead('lead-stale-receipt'))
