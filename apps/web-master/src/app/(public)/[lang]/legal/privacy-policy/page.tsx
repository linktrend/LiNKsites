...
export async function generateMetadata({ params }: Props) {
  ...
  try {
    const { page, locale } = await loadLegalFamilyPage(lang, "privacy-policy");
...
export default async function PrivacyPolicyPage({ params }: Props) {
  const { lang } = await params;
  const { page, siteId, locale, template, runtime } = await loadLegalFamilyPage(lang, "privacy-policy");
  return (
