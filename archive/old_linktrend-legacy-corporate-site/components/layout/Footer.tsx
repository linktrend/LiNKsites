'use client';

import { useState } from 'react';
import Link from 'next/link';
import { Sparkles } from 'lucide-react';
import { CookiePreferencesModal } from '@/components/modals/CookiePreferencesModal';

const socialLinks = [
  {
    name: 'LinkedIn',
    href: 'https://linkedin.com',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
      </svg>
    ),
  },
  {
    name: 'Twitter/X',
    href: 'https://twitter.com',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
      </svg>
    ),
  },
  {
    name: 'GitHub',
    href: 'https://github.com',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
      </svg>
    ),
  },
];

const sitemapColumns = {
  platform: {
    title: 'Platform',
    links: [
      { label: 'Advantage 1', href: '/platform/advantage-1' },
      { label: 'Advantage 2', href: '/platform/advantage-2' },
      { label: 'AI', href: '/platform/ai' },
      { label: 'Innovation & Support', href: '/platform/innovation-support' },
      { label: 'Pricing', href: '/pricing' },
      { label: 'Enterprise', href: '/enterprise' },
    ],
  },
  solutions: {
    title: 'Solutions',
    links: [
      { label: 'Customers', href: '/solutions/customers' },
      { label: 'Industry', href: '/solutions/industry' },
      { label: 'Company Size', href: '/solutions/company-size' },
      { label: 'Role', href: '/solutions/role' },
    ],
  },
  resources: {
    title: 'Resources',
    links: [
      { label: 'Library', href: '/resources/library' },
      { label: 'For Customers', href: '/resources/for-customers' },
      { label: 'About Us', href: '/about' },
      { label: 'Contact Us', href: '/contact' },
    ],
  },
};

const APP_URL = 'http://localhost:3000';

export default function Footer() {
  const [isCookieModalOpen, setIsCookieModalOpen] = useState(false);

  return (
    <>
      <footer
        className="mt-auto border-t text-white"
        style={{ backgroundImage: 'var(--gradient-surface-footer)' }}
      >
        <div className="container mx-auto px-8 py-12">
          {/* Top Section: Logo + Sitemap */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-8">
            {/* Logo and Name */}
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-2 text-white">
                <Sparkles className="h-6 w-6" />
                <span className="text-lg font-bold">LTM Starter Kit</span>
              </div>
            </div>

            {/* Blank Column */}
            <div></div>

            {/* Platform Column */}
            <div>
              <h3 className="font-semibold text-sm mb-4 text-white">{sitemapColumns.platform.title}</h3>
              <ul className="space-y-3">
                {sitemapColumns.platform.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-white/70 hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Solutions Column */}
            <div>
              <h3 className="font-semibold text-sm mb-4 text-white">{sitemapColumns.solutions.title}</h3>
              <ul className="space-y-3">
                {sitemapColumns.solutions.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-white/70 hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Resources Column */}
            <div>
              <h3 className="font-semibold text-sm mb-4 text-white">{sitemapColumns.resources.title}</h3>
              <ul className="space-y-3">
                {sitemapColumns.resources.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-white/70 hover:text-white transition-colors"
                    >
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </div>

          {/* Social Media Icons Bar */}
          <div className="flex items-center gap-6 py-6">
            <div className="flex-grow border-t border-white/30"></div>
            <div className="flex justify-center items-center gap-6">
              {socialLinks.map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white/70 hover:text-white transition-colors"
                  aria-label={social.name}
                >
                  {social.icon}
                </a>
              ))}
            </div>
            <div className="flex-grow border-t border-white/30"></div>
          </div>

          {/* Bottom Section: Copyright + Legal Links */}
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 text-sm text-white/70">
            <div>
              <p>&copy; Copyright 2025. LiNKtrend Media</p>
            </div>
            <div className="flex items-center gap-6">
              <Link href="/terms" className="hover:text-white transition-colors">
                Terms of Use
              </Link>
              <Link href="/privacy" className="hover:text-white transition-colors">
                Privacy Policy
              </Link>
              <button
                onClick={() => setIsCookieModalOpen(true)}
                className="text-sm hover:text-white transition-colors"
              >
                Manage Cookies
              </button>
              <a href={`${APP_URL}/console/login`} className="hover:text-white transition-colors font-semibold">
                Admin Login
              </a>
            </div>
          </div>
        </div>
      </footer>

      {/* Cookie Preferences Modal */}
      <CookiePreferencesModal
        isOpen={isCookieModalOpen}
        onClose={() => setIsCookieModalOpen(false)}
      />
    </>
  );
}
