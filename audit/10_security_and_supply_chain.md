...
- **Remaining medium/low findings — RESOLVED (2026-07-14, GAP-49, PR #46):** the ~100 medium / ~31 low findings were triaged and patched: Next.js bumped to 16.2.10 (several required ≥16.2.5), `next-intl` bumped to `^4.9.2`, root `turbo` bumped to `^2.10.5`, and narrow same-major-bounded overrides added for `dompurify`, `brace-expansion`, `picomatch` (two ranges), `postcss`, `qs`, `uuid`, and `icu-minify`.
- **Legacy app archived (2026-07-14, PR #45), per Carlos's explicit instruction:** `sites_projects/old_linktrend` (the source of ~66 of the 222 findings) was moved to `archive/old_linktrend-legacy-corporate-site/`, its lockfiles removed, and its `package.json` renamed so GitHub's dependency graph no longer scans it. This resolves those findings by removing the scanned artifact rather than patching a dependency tree that was never actually installed or run.
- **React 18/19 alignment resolved (2026-07-14, GAP-46, PR #44), per Carlos's explicit instruction:** `apps/web-master` and `apps/web-company` upgraded to React 19.1.0, matching `apps/cms`. `next build` now succeeds for both apps (previously blocked — see GAP-46 history above). This also fixed a second, previously-hidden async-`params` build blocker in `apps/web-company`, caught only by actually running `next build`, not by `tsc --noEmit`.
- **Residual, accepted risk (documented, not chased further):** for a handful of packages (`vite`, `undici`, `ws` from GAP-47; `uuid`, `icu-minify`, `dompurify` from GAP-49), pnpm's dependency tree retains older, still-vulnerable instances coexisting with the newly-patched one, for different transitive consumer contexts deep in the tree. All are dev-tooling-only dependencies never shipped to production. Forcing full pnpm-store deduplication (e.g., via `resolutionMode` changes or per-consumer override entries) was judged disproportionate effort relative to actual risk, given none of these are reachable by production request paths. Re-evaluate if this changes (e.g., if any of these ever become a runtime, request-path dependency).
- **Net result:** as of 2026-07-14, all 222 originally-found Dependabot alerts have been addressed by one of: a direct version/dependency fix, a version-truth correction (declared version now matches reality), or removal of the scanned artifact (legacy app archived). This should be re-verified once PRs #38-#46 merge to `development`/`main` and GitHub rescans — the counts above are based on manual triage of the alert API, not a live rescan post-merge.
