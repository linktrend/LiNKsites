...
const required = ['LINKLIBRARIES_CATALOG_SHA', 'LINKLIBRARIES_ENTRY_SHA', 'LINKSITES_CMS_IMAGE_DIGEST', 'LINKSITES_WEB_MASTER_IMAGE_DIGEST', 'LINKSITES_ORCHESTRATOR_IMAGE_DIGEST', 'LINKSITES_WORKER_IMAGE_DIGEST']
...
for (const name of ['LINKLIBRARIES_CATALOG_SHA', 'LINKLIBRARIES_ENTRY_SHA']) if (!/^[a-f0-9]{40,64}$/i.test(process.env[name])) throw new Error(`${name} must be an immutable SHA`)
...
const manifest = {
  ...
  images: { cms: process.env.LINKSITES_CMS_IMAGE_DIGEST, webMaster: process.env.LINKSITES_WEB_MASTER_IMAGE_DIGEST, orchestrator: process.env.LINKSITES_ORCHESTRATOR_IMAGE_DIGEST, autoworkWorker: process.env.LINKSITES_WORKER_IMAGE_DIGEST },
