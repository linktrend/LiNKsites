...
const hookLines = [
  ...
  'test -f "$LINKSITES_LOCAL_PROOF_ROOT/state/program-ledger.json" || die "durable ledger is missing"',
  'grep -q "\\\"completion.emitted\\\"" "$LINKSITES_LOCAL_PROOF_ROOT/state/program-ledger.json" || die "completion event missing"',
  'grep -q "\\\"delivered\\\"" "$LINKSITES_LOCAL_PROOF_ROOT/state/program-ledger.json" || die "delivered outbox state missing"',
  ...
  'grep -q "\\\"completion.emitted\\\"" "$LINKSITES_LOCAL_PROOF_ROOT/state/program-ledger.json" || die "completion was not restored"',
  'grep -q "\\\"delivered\\\"" "$LINKSITES_LOCAL_PROOF_ROOT/state/program-ledger.json" || die "outbox was not restored"',
