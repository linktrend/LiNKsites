...
const hookLines = [
  ...
  'next_lock="$root/apps/cms/.next/dev/lock"', 'next_lock_pid=""',
  "if test -f \"$next_lock\"; then next_lock_pid=\"$(node -p 'const value = JSON.parse(require(\"node:fs\").readFileSync(process.argv[1], \"utf8\")); if (!Number.isInteger(value.pid) || value.pid < 2) process.exit(2); value.pid' \"$next_lock\")\" || die \"CMS development lock is malformed\"; if kill -0 \"$next_lock_pid\" >/dev/null 2>&1; then is_cms_descendant \"$next_lock_pid\" || die \"refusing to remove development lock not owned by the disposable CMS launcher\"; else next_lock_pid=\"\"; fi; fi",
  'cms_descendants() { for child in $(pgrep -P "$1" 2>/dev/null || true); do cms_descendants "$child"; printf "%s\\n" "$child"; done; }', 'cms_tree="$(cms_descendants "$LINKSITES_LOCAL_PROOF_CMS_PID")"',
  ...
  'sleep 2', 'mkdir -p "$restore"', 'test -d "$restore" || die "post-restart readback directory is unavailable"', 'payload_body="$restore/payload-readback.json"', ': > "$payload_body" || die "could not create post-restart Payload readback file"', 'payload_status="$(curl --globoff --retry 4 --retry-connrefused --silent -o "$payload_body" -w "%{http_code}" -H "Authorization: users API-Key $LINKSITES_LOCAL_PROOF_API_KEY" "http://127.0.0.1:$LINKSITES_LOCAL_PROOF_CMS_PORT/api/pages?site=$LINKSITES_LOCAL_PROOF_SITE_ID&limit=100" || true)"', 'test "$payload_status" = 200 || die "Payload readback HTTP status $payload_status (body bytes $(wc -c < "$payload_body" 2>/dev/null || echo 0))"', 'node - "$payload_body" "$LINKSITES_LOCAL_PROOF_RUN_MARKER" "$diagnostic_root/payload-readback-summary.json" <<\'NODE\'', 'const fs = require("node:fs")', 'const [bodyPath, marker, summaryPath] = process.argv.slice(2)', 'const value = JSON.parse(fs.readFileSync(bodyPath, "utf8"))', 'const all = Array.isArray(value.docs) ? value.docs : []', 'const matches = all.filter((doc) => doc && doc.promotionRunMarker === marker)', 'fs.writeFileSync(summaryPath, JSON.stringify({ keys: Object.keys(value).sort(), totalDocs: all.length, matchCount: matches.length, docs: all.map((doc) => ({ id: doc?.id ?? null, site: doc?.site?.id ?? doc?.site ?? null, marker: doc?.promotionRunMarker ?? null, status: doc?.status ?? null, versionStatus: doc?._status ?? null, previewEnvironment: doc?.previewEnvironment ?? null })) }, null, 2) + "\\n")', 'NODE', "node -e 'const value = JSON.parse(require(\"node:fs\").readFileSync(process.argv[1], \"utf8\")); const marker = process.argv[2]; const all = value.docs; const docs = Array.isArray(all) ? all.filter((doc) => doc && doc.promotionRunMarker === marker) : []; if (docs.length !== 5 || new Set(docs.map((doc) => doc.id)).size !== 5 || docs.some((doc) => !doc.id || doc.promotionRunMarker !== marker || doc.status !== \"published\" || doc._status !== \"published\")) process.exit(1)' \"$payload_body\" \"$LINKSITES_LOCAL_PROOF_RUN_MARKER\" || die \"Payload readback did not retain five unique published restored page IDs\"",
  ...
  // As with the CMS, the old web listener may already have exited by the
  // recovery boundary.  Treat absence as stopped, but retain the ownership
  // check for every listener that remains.
  'web_listener_pids="$(lsof -nP -tiTCP:"$LINKSITES_LOCAL_PROOF_WEB_PORT" -sTCP:LISTEN || true)"', 'for web_listener_pid in $web_listener_pids; do is_web_descendant "$web_listener_pid" || die "refusing to stop listener not owned by disposable web-master launcher"; done',
...
const webStartIndex = hookLines.findIndex((line) => line.includes('web_payload_api_key_env'))
if (webStartIndex < 0) throw new Error('recovery web start command is missing')
hookLines.splice(webStartIndex, 0, 'web_payload_api_key_env=PAYLOAD_API_KEY', 'web_preview_token_env=PREVIEW_ACCESS_TOKEN')
const hookSource = hookLines.join('\n')

try {
  await writeFile(hook, hookSource, { mode: 0o700 })
