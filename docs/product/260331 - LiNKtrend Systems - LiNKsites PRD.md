...
### **5.4.1 CMS Seed and Bootstrap Requirements**

To validate the platform and avoid false negatives in Phase 2 and Phase 3 testing, the CMS must include a deterministic bootstrap dataset. This dataset must include at minimum:

• One default Site with domain and locale mapping
• Navigation structures (primary + footer)
• A homepage, contact page, and legal pages
• Global settings (SEO defaults, header/footer, legal settings)

Seed data must be scripted or importable (not manual), and must be repeatable across environments.
...
### **5.4.2 Access Control, Public Read, and Auditability**

The CMS must explicitly define which collections are public-read and which require API keys or role-based access. The policy must include:

• A public-read allowlist of collections
• API-key or token requirements for private collections
• Audit logging for content mutations and automation writes
• Rate limiting and basic abuse protection for public endpoints

This ensures that the CMS can safely serve multi-site content while maintaining governance and traceability.
...
### **6.4.1 Data Contract Artifacts (Required)**

Every schema change must update a machine-readable data contract package that is versioned in-repo and consumed by CMS, frontend, and automation. This package must include:

• Canonical TypeScript types for CMS collections and shared blocks
• Canonical Supabase table definitions (SQL or schema JSON)
• Mapping tables between Supabase fields and CMS collections
• Version stamps for backward compatibility and migration notes

The data contract artifacts must be generated or updated as part of the same commit that changes any schema, and must be used by validation tests to prevent drift.
...
### **6.6.1 Dependency Conflict Mitigation**

To prevent dependency conflicts in a unified monorepo:

• Standardize on a single package manager (pnpm) with workspace constraints
• Centralize version policies in the root (e.g., enforced versions for React, Next.js, Payload, Tailwind)
• Use "overrides" or "resolutions" in root package.json to pin critical versions
• Disallow duplicate framework instances across apps via workspace lint rules
• Maintain a shared tsconfig base and ESLint config to avoid divergent build expectations

If a conflict is unavoidable, the resolution must be documented in a repo-level CHANGELOG entry describing the decision, scope, and rollback plan.
...
### **6.9.1 Schema Versioning and Rollback Strategy**

All schema changes must be versioned and reversible:

• Every schema migration must include a down/rollback path
• Breaking changes must include a compatibility window and deprecation notice
• Release tags must identify the schema version, CMS build, and frontend build in lockstep

Rollback procedures must be tested during Phase 1 validation and re-validated during Phase 2.
...
### **6.10.1 Monorepo Transition Execution Plan**

The transition from the current dual-repo structure to the Turborepo monorepo must be executed in a deterministic sequence:

1. Create root workspace with Turborepo tooling and base configs (pnpm-workspace.yaml, turbo.json, base tsconfig/eslint).
2. Move CMS into /apps/cms and master template into /apps/web (or /apps/templates/web-master if multiple web apps are maintained).
3. Extract shared logic into /packages (types, ui, blocks, config, utils).
4. Add /supabase directory containing migrations, functions, and schema definitions.
5. Update all imports to consume shared packages rather than local copies.
6. Validate build and type integrity with monorepo-level commands.

### **6.10.2 Target Folder Tree (Required End State)**

/apps
  /cms
  /web-master
  /web-company (optional, if maintained as separate app)
/packages
  /types
  /ui
  /blocks
...
### **6.10.3 Centralization Opportunities Enabled by Monorepo**

Once consolidated, the following must be centralized for consistency and maintainability:

• Shared design tokens and theming configuration
• Shared CMS block definitions and rendering logic
• Shared SEO and structured data utilities
• Shared localization and routing logic
• Shared environment variable contracts and validation
• Shared release/versioning process for schema changes

This centralization reduces drift, lowers maintenance cost, and makes AI-executed changes safer and more predictable.
...
### **7.3.5 Language Canonicalization and Migration**

The canonical languages for LiNKsites are: English (en), Traditional Chinese (zh-tw), and Spanish (es).

If additional locales are present in the current system (e.g., zh-cn, fr, de), they must be treated as non-canonical until explicitly promoted. The platform must define a migration plan to either:

• map non-canonical locales to canonical ones, or
• formally add them to the canonical set with full CMS schema, translation workflows, and QA coverage.

All language support must be consistent across CMS, templates, SEO metadata, and automation pipelines before Phase 3 can be considered complete.
...
### **7.6.7 Implementation Constraints and Risk Mitigation (v4.0)**

• AI-bot Markdown views must preserve content parity with human pages to avoid SEO cloaking risks. Use a content-equivalent renderer that strips chrome but preserves full text and metadata.
• Referrer-based UI changes must not alter core factual content. All variants must share the same canonical content blocks.
• ai-actions.json must be versioned, validated, and secured with rate limits and auth tokens for any state-changing action.
• llms.txt and robots.txt must be generated from the same source-of-truth to avoid contradictory crawling guidance.
• “Content-Signal: ai-train=yes” is non-standard; it must be treated as optional and require legal/privacy review before enabling.
• “Positive-only trust feed” must comply with review platform terms; fallback to hide the module if filtering violates policy.
• Cloudflare proxy + tunnel must be documented with zero-trust inbound rules and tested against staging domains.
...
### **9.9.1 Error-State Matrix (Mandatory)**

The following failure states must be explicitly handled:

• Content Generation Failure → retry job, mark Supabase state as generation_failed, notify LiNKbot
• Supabase Write Failure → retry with backoff, log failure, do not proceed to CMS sync
• CMS Sync Failure → mark sync_failed, keep Supabase as source of truth, re-run sync only
• Deployment Failure → mark deploy_failed, keep CMS content intact, allow manual retry
• CRM Update Failure → mark crm_update_failed, re-run CRM update only

Each failure must have a deterministic retry policy, logging requirements, and a clear owner (automation vs LiNKbot) for recovery.
...
### **13.11.1 Performance KPIs (Mandatory Targets)**

The platform must meet the following performance targets in production mode for the master template:
...
These targets must be validated in CI using a standard test profile and must be recorded per release.
...
- turbo run typecheck
- turbo run build
- verify shared types package compiles and is consumed by CMS + frontend  
...
- run CMS schema diff vs shared types
- run Supabase migration validation
- run sample Supabase → CMS sync with seeded data  
...
- render CMS-driven pages in all canonical locales
- validate JSON-LD output against schema.org validators
- run Lighthouse audit to performance KPIs
...
- render industry template pages with seeded data
- verify schema mapping completeness for each industry
- run automation compatibility checks on content fields
...
- run end-to-end pipeline with dummy CRM record
- verify Supabase write, CMS sync, and CRM update logs
- confirm retry behavior on simulated failures
...
- deploy CMS and frontend to staging droplet
- verify hostname-based routing in shared mode
- run post-deploy smoke tests and monitoring checks
