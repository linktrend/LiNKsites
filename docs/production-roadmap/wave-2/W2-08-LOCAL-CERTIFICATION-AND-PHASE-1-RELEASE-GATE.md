...
**Status:** Pre-VPS hardening candidate pending Luna High verification, hosted required checks, and protected-branch promotion; separate Phase 2 VPS authority remains on HOLD
**Wave:** 2 final checkpoint
**Executor:** Luna High verifier; Luna High agents execute correction packets
...
5. Luna High verifies the frozen SHA against every packet acceptance criterion and the repository evidence. A failed criterion creates a narrow Luna High correction packet followed by Luna High re-verification.
...
- Luna High verification and all required hosted checks pass at the same exact SHA;
...
## Luna High verification and correction loop

Freeze the candidate and have Luna High verify it against the Phase 1 requirements and the exact hosted-check result. Any failed requirement is not completion. Convert each finding to an atomic correction Issue, assign non-overlapping Luna High agents, integrate, rerun affected hosted checks, and have Luna High re-verify the new SHA.
