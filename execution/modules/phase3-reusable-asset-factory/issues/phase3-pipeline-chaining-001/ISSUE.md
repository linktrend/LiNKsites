...
objective: "Address the open gap named in both execution/modules/phase3-reusable-asset-factory/MODULE.md and audit/14_implementation_roadmap.md -- 'Automatic pipeline chaining (Site Specification -> Site Assembly -> Promotion as one flow) -- each remains an independently dispatchable Issue type for now.' -- with the real, honest, buildable slice of that gap: type-safe mapping/builder functions that take one Phase's accepted output plus the caller-supplied real data the next Phase genuinely needs, and construct the next Phase's Issue input with all identifier-propagation fields (siteSpecId, siteId, kitId, manifestId, assemblyManifestId, targetSiteId, etc.) copied correctly and validated for basic sanity."
scope:
...
out_of_scope:
  - "A fully autonomous, auto-triggering pipeline that decides WHEN to advance from one Phase to the next -- that remains a distinct Gate-acceptance authority (manual doctrine) these functions do not touch"
