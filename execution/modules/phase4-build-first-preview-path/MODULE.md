...
scope:
  ...
  - "Conversion Lock: locks a Foundation relationship on LiNKreach-authorized conversion, blocking recycling (done: phase4-conversion-lock-001, manual §10.33); legacy Stripe/Odoo reference fields remain opaque compatibility data, not integrations"
  ...
  - "Outcome Record + LiNKreach-outcome-to-technical-disposition mapping: the single, deterministic, type-enforced mapping from LiNKreach's commercial outcome vocabulary to LiNKsites' technical action vocabulary (done: phase4-outcome-record-001, manual §10.31); the existing SalesOutcome symbol is a compatibility name"
...
out_of_scope:
  ...
  - "A real LiNKreach-owned payment/CRM adapter behind Conversion Lock's opaque compatibility refs -- blocked on cross-Program access (GAP-33/34/35)"
  - "The manual's full seven-object inventory model -- no real deployed-preview or LiNKreach-outcome integration exists to back further objects beyond what's built"
phases: []
read_first:
...
- A real LiNKreach-owned payment/CRM adapter behind Conversion Lock's opaque compatibility refs -- blocked on cross-Program access (GAP-33/34/35).
- The manual's full seven-object inventory model, cost/conversion/coverage-rate portfolio metrics, and full multi-factor Foundation ranking -- all need data (cost ledgers, LiNKreach outcome telemetry, engagement history) this repository does not have.
