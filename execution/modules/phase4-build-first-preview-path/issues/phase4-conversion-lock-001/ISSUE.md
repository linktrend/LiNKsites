...
title: "Conversion Lock: block recycling/reassignment of a Foundation once LiNKreach authorizes conversion of a published preview"
status: "review_ready"
...
out_of_scope:
  - "Any live LiNKreach-owned payment or CRM integration -- the existing stripePaymentConfirmationRef and odooCommercialRecordRef fields are opaque, caller-supplied historical compatibility references, never verified against an external payment API or CRM record (GAP-33/34/35, a previously-identified blocker requiring cross-Program access this repository does not have)"
  - "Real LiNKreach-authorization validation for conversionInstructionRef -- same honest-scope boundary as other Issues in this session that reference authority_ref/proof_authority_ref"
...
review_requirements:
  - "Independent reviewer confirms the doc comments' scope claims match what the code actually does -- no overclaiming of payment/CRM verification, LiNKreach-authorization validation, or downstream finalization coverage beyond the lock and its recycle-blocking gate"
