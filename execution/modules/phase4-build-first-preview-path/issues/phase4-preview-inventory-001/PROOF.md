...
optional_fields:
  ...
  open_gaps:
    - "This Issue implements only the subset of manual §10.38's portfolio metrics that are computable from data that genuinely exists today (foundation counts by status/kit/tier, reservation utilization, adaptation-status breakdown, foundation reuse distribution). Cost per conversion, conversion rate by family, coverage rate for qualified leads, and average time to preview readiness all require data -- cost ledgers, LiNKreach outcomes, readiness timestamps -- that does not exist in this repository yet."
