...
test('queue integrity secret is mandatory, stored records are untrusted, grants are rechecked, and metrics survive restart', async () => {
  ...
  const healthy = new FileOutbox(healthyPath, options)
  await healthy.enqueue(gateway.buildRequest('demo.completed', 'org_demo', 'corr', 'healthy-1', payload))
  await healthy.drain(async (request) => ({ status: 202, receiptId: 'receipt-healthy', receiptSignature: gateway.signAcknowledgement(request, 'receipt-healthy', clock.nowIso()), acknowledgedAt: clock.nowIso() }), 1_000)
  const restarted = new FileOutbox(healthyPath, options)
