...
import type { PayloadDraftTarget } from '../promotionService'
...
export class PayloadRestDraftTarget implements PayloadDraftTarget {
  ...
  async publishPrivate(payloadDocumentId: string, publicationMarker: string, owningSiteId: string): Promise<{ published: boolean; readback: Record<string, unknown> }> {
    ...
    const response = await fetch(url, { method: 'PATCH', headers: { 'Content-Type': 'application/json', ...this.buildHeaders() }, body: JSON.stringify({ _status: 'published', status: 'published', previewEnvironment: 'private-preview', publicActivation: false, promotionRunMarker: publicationMarker }) })
    ...
    if (!readback || readback._status !== 'published' || readback.status !== 'published' || readback.previewEnvironment !== 'private-preview' || readback.promotionRunMarker !== publicationMarker || readback.publicActivation !== false || String(readbackSite) !== owningSiteId) throw new Error('payload private publication readback failed')
