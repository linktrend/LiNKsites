...
describe('Revision 2 website-template materialization', () => {
  ...
  it('materializes the protected uncatalogued A1 candidate into an out-of-repo cache, then survives checkout removal, tamper, and rollback', async () => {
    ...
    try {
      ...
      expect(first.ok, !first.ok ? first.errors.join(' | ') : '').toBe(true)
      if (!first.ok) return
      for (const catalogueFileSha256 of [undefined, '0'.repeat(64)]) {
        const rejected = materializeRevision2WebsiteTemplate({
          providerRoot, entryId: MASTER_TEMPLATE_PIN.entryId, version: MASTER_TEMPLATE_PIN.version,
          pin: { ...pin, catalogueFileSha256 }, selectionPolicy: 'draft_candidate_probe',
        })
        expect(rejected.ok).toBe(false)
      }
