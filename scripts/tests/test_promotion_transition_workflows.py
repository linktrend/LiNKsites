...
class PromotionTransitionWorkflowTests(unittest.TestCase):
    def test_protected_promotion_workflows_require_transition_receipts(self) -> None:
        for name, branch, _target in MANAGED_PROMOTION:
            ...
            self.assertIn('git ls-remote --exit-code origin "refs/heads/${TARGET_BRANCH}"', text)
            self.assertEqual(text.count('git ls-remote --exit-code origin "refs/heads/${TARGET_BRANCH}"'), 2)
            self.assertIn('test "${BASE_SHA}" = "${EVENT_BASE_SHA}"', text)
            self.assertIn("receipt_gate_base_moved", text)
            self.assertIn("receipt_gate_base_moved_during_run", text)
            self.assertIn("receipt_gate_base_tree_moved_during_run", text)
            self.assertIn("receipt_gate_source_pr_moved_during_run", text)
            self.assertLess(text.index("receipt_gate_base_moved_during_run"), text.index("promotion_receipt_gate.py\" authorize"))
            self.assertLess(text.index("receipt_gate_base_tree_moved_during_run"), text.index("receipt_gate_source_pr_moved_during_run"))
            self.assertLess(text.index("receipt_gate_source_pr_moved_during_run"), text.index("promotion_receipt_gate.py\" authorize"))
            self.assertIn('gh api "repos/${GITHUB_REPOSITORY}/pulls/${SOURCE_PR}" --jq .head.sha', text)
            self.assertIn('test "${latest_source_head}" = "${source_head}"', text)
            self.assertIn("--expected-base-commit", text)
            self.assertIn("--expected-base-tree", text)
